...
// 실시간 감정 체크인 트리거
export const onEmotionCheckin = onDocumentCreated({
...
// 지식 그래프 업데이트 트리거
export const onKnowledgeUpdate = onDocumentUpdated({
...
export const realtimeUpdates = onRequest({
  region: 'us-central1',
  cors: true
}, async (req, res) => {
        ...
        if (change.type === 'added') {
          ...
          res.write(`data: ${JSON.stringify({
            ...
            data: {
              id: change.doc.id,
              ...data,
              generated_at: data.generated_at?.toDate?.()?.toISOString() || new Date().toISOString()
            }
          })}\n\n`);
        }
      });
...
// 실시간 감정 패턴 분석
async function triggerEmotionAnalysis(userId: string) {
  const db = admin.firestore();
...
// 지식 그래프 업데이트 브로드캐스트
async function broadcastKnowledgeUpdate(nodeId: string, data: any) {
  const db = admin.firestore();
...
// 초기 실시간 데이터 로드
async function getInitialRealtimeData(userId: string) {
  const db = admin.firestore();
  ...
  return {
    emotion: recentEmotion.docs[0]?.data() || null,
    insight: recentInsight.docs[0]?.data() || null,
    timestamp: new Date().toISOString()
  };
}

// 감정 패턴 분석 함수
function analyzeEmotionPatterns(emotions: any[]) {
...
// 맞춤형 추천 생성
function generateRecommendations(dominant: string, distribution: any): string[] {
...
// 변경사항 감지 헬퍼
function getChanges(before: any, after: any): any {
  const changes: any = {};
